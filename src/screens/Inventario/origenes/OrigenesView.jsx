...
import { resetData, origenesData,origenesDelete, origenesUpdate, origenesCreate, resetItem } from '../../../reducers/slices/origenSlice'
...
const OrigenesView = () =>{
    ...
    const { data, total, pagina, paginas, loading } = useSelector(state => state.origen)     
    ...
    const handleDelete = (pky) =>{     
        dispatch(origenesDelete({origenId:pky})) 
    }   
    ...
    const submitHandle = event =>{               
        ...
        if(nitem.id){            
            dispatch(origenesUpdate(nitem)) 
        }else{            
            dispatch(origenesCreate(nitem)) 
        }
      handleClear()
    }
