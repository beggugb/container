...
import { resetData,tiposData,tiposDelete, tiposUpdate, tiposCreate, resetItem } from '../../../reducers/slices/tipoSlice'
...
const TiposView = () =>{
    ...
    const { data, total, pagina, paginas, loading } = useSelector(state => state.tipo)     
    ...
    const handleDelete = (pky) =>{     
        dispatch(tiposDelete({tipoId:pky})) 
    }   
    ...
    const submitHandle = event =>{               
        ...
        if(nitem.id){            
            dispatch(tiposUpdate(nitem)) 
        }else{            
            dispatch(tiposCreate(nitem)) 
        }
      handleClear()
    }
